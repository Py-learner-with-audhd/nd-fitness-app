...
5. **"+ Type your own" on any slot (added 2026-08-15)** — Emmet clarified choice overwhelm isn't actually his friction point here; the real one is not wanting to keep a separate paper logbook for anything outside the planned pool. Typing a name adds it to that slot's pool permanently (`is_default: 0`) — it's only typed once, then behaves exactly like any other tap option, and progress tracking works on it immediately since it's a real variation row, not a disposable free-text field.
